...
public class JenkinsClient
{
	...
	private static String BASE_URL = "http://buildmaster-hyd.opentext.net/jenkins/view/";
	
	private static String BASE_URL_MACHINE = "http://buildmaster-hyd.opentext.net/jenkins/computer/";
	...
	private Properties machines;

	public JenkinsClient (String view, Properties jobOwners, Properties machines)
	{
	...
	public List<Job> getViewJobs ()
	{
		...
		for(Job job : jobs.getJobs())
		{
			...
			job.setMachineName(gettingMachineName(job.getName()));
			job.setMachineurl(BASE_URL_MACHINE+job.getMachineName());
	...
	private String gettingMachineName(String JobName) {
		target = client.target(BASE_URL).path(this.view).path("/job/").path(JobName).path(LAST_BUILD).path(API);
		response = target.request(MediaType.APPLICATION_JSON).get();
		
		LastBuild lastBuild = response.readEntity(LastBuild.class);
		System.out.println(lastBuild.getBuiltOn());
	...
	public List<TestJob> getTestJobs(List<Job> jobs)
	{
		...
		for(Job job : jobs)
		{
			...
			testjob.setMachineName(gettingMachineName(job.getName()));
			testjob.setMachineurl(BASE_URL_MACHINE+job.getMachineName());
